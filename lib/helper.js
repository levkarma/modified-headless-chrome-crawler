...
class Helper {
  ...
  static generateKey(options) {
    // lev
    if (options.ignoreInternalAnchors) options.url = options.url.split('#')[0]
    if (options.ignoreUrlParameters) options.url = options.url.split('?')[0]
    for (let string of options.ignoreUrlsContaining) { if (options.url.includes(string)) return }
    // 
