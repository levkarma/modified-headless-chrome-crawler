...
const DYNAMIC_OPTIONS_FILE_PATH = __dirname + "/../dynamic-options.json";
...
class HCCrawler extends EventEmitter {
  ...
  async _checkRequested(options) {
    if (!options.skipDuplicates) return false;
        // lev
    const dynamicOptions = await asyncReadFile(DYNAMIC_OPTIONS_FILE_PATH,'utf8')
    for (let string of JSON.parse(dynamicOptions).ignoreUrlsContaining) { 
      if (options.url.includes(string)) {
        console.log('Skipping because includes ' + string)
        return true 
      }
    }
